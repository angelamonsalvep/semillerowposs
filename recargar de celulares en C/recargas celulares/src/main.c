...
int validatePassFormat()
{
	...
	//se muestra mensaje si la contrase�a tiene menos de 8 caracteres
	if(i<8)
	{
		messageInvalidPass;
		return 1;
...
void messageInvalidKey()
{
	...
	printf("\n\nOprima la tecla Enter para seleccionar una opci�n valida\n\n");
	while(key=getch())
	{
		if(key==13)
		{
...
char* askForCellPhoneNumber(char cellPhoneNumber[])
{
	...
	{
		printf("\n\nIngresa el n�mero de celular a recargar, recuerda que debe tener 10 digitos: \n\n");
	}
	else
	{
		printf("\n\nEscribe nuevamente el n�mero de celular para confirmar: \n\n");
	}
...
void validateNumberFormat(char cellNumber1[], char cellNumber2[])
{
	...
	printf("RECARGAS - INGRESAR N�MERO DE CELULAR A RECARGAR");
	...
	while(cell1equalcell2!=0)
	{
		...
		printf("\n\nOprima la tecla enter para volver a ingresar el n�mero de celular a recargar\n\n");
	...
	{
		countAskForCellNumber=0;
		printf("\n\nSe ha validado correctamente el n�mero de celular.\n\n");
...
void ShowRechargeConfirmation()
{
	...
	printf("\n\nOprima la tecla enter para volver al men� principal\n\n");
		while(key=getch())
		{
			if(key==13)
			{
...
void registerUser()
{
	...
	printf("\n\nOprima la tecla enter para Iniciar Sesi�n\n\n");
		while(key=getch())
		{
			if(key==13)
			{
